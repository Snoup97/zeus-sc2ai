import sc2
from sc2.constants import NEXUS, PROBE, PYLON, ASSIMILATOR, GATEWAY, CYBERNETICSCORE, WARPGATE, ROBOTICSFACILITY
...
async def build_assimilators(self):
    for nexus in self.units(NEXUS).ready:
        ...
        for vaspene in vaspenes:
            ...
            if self.can_afford(ASSIMILATOR) and self.units(NEXUS).amount > 1 and self.units(ASSIMILATOR).amount < 3:
                worker = self.select_build_worker(vaspene.position)
                await self.do(worker.build(ASSIMILATOR, vaspene))
